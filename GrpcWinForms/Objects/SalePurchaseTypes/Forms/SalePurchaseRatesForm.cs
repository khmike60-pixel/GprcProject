...
namespace GrpcWinForms.Objects.SalePurchaseTypes.Forms
{
    public partial class SalePurchaseRatesForm : Form, ISalePurchaseRatesView
    {
        ...
        public event Func<SalePurchaseGridRate, CancellationToken, Task> OnCommitEditAsync;
        public event Func<IReadOnlyList<int>, CancellationToken, Task> OnDeleteAsync;
        public event Func<SalePurchaseGridRate, CancellationToken, Task> OnAppendAsync;
        ...
        public SalePurchaseRatesForm()
        {
            ...
            btnDelete.Click += async (s, e) =>
            {
                ...
                foreach (int r in gridRates.SelectedRows)
                {
                    ...
                    var item = gridRates.Rows[r].DataSource as SalePurchaseGridRate;
                    if (item?.Id != null) ids.Add(item.Id.Value);
                }
            ...
            {
                var item = new SalePurchaseGridRate();
                if (gridRates.Row > gridRates.Rows.Fixed) 
                    item = gridRates.Rows[gridRates.Row].DataSource as SalePurchaseGridRate;
                if (item == null) return;
        ...
        private async void GridSalePurchaseRates_AfterEdit(object sender, RowColEventArgs e)
        {
            try
            {
                ...
                var item = gridRates.Rows[e.Row].DataSource as SalePurchaseGridRate;
                if (item == null) return;
        ...
        public void ShowRates(BindingList<SalePurchaseGridRate> rates)
        {
